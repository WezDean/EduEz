def predictions():
    ...
    with tab1:
        ...
        # Feature Engineering with updated 'Parent_Education_Level' to include O-Level and A-Level
        ...
        # Expanding Parent Education Levels to include O-Level and A-Level
        ...
        with col3:
            ...
            # User input for expanded 'Parent's Education Level'
            parent_education = st.selectbox("Parent's Education Level", list(parent_education_map.keys()))
        ...
        user_input = pd.DataFrame({
            ...
            'Parent_Education_Level': [parent_education_map[parent_education]],  # New mapping for O/A Level
    ...
    with tab2:
        ...
        # Display model evaluation and feature importance in columns
        col1, col2 = st.columns(2)

        # Model Evaluation in col1
        with col1:
        ...
        with col2:
            ...
            # Simple feature importance chart
            fig, ax = plt.subplots(figsize=(6, 3))
            plot_importance(best_xgb_model_credits, ax=ax, height=0.5, max_num_features=5)
            plt.tight_layout()
            plt.grid(False)
            st.pyplot(fig)
        ...
        with col2:
            age = st.number_input("Age", min_value=16, max_value=25, key='age_credits')
        ...
        study_hours = st.slider("Weekly Study Hours", 0, 40, 10, key='study_hours_credits')
        ...
        user_input_credits = pd.DataFrame({
            ...
            'Parent_Education_Level': [parent_education_map[parent_education]],  # Updated with new options
            'Study_Hours': [study_hours],
            'Extracurricular_Activities': [1 if extracurricular_activities == 'Yes' else 0]
        }, columns=features_credits)
